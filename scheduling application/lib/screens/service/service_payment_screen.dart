...
// SỬ DỤNG IMPORT ĐÃ CÓ VÀ ĐÃ HOẠT ĐỘNG
import 'package:vietqr_gen/vietqr_generator.dart'; 
import '../../models/health_package.dart';

// Ghi chú: Dựa trên code mẫu mới của bạn, tôi giả định thư viện 
// vietqr_gen của bạn hỗ trợ Bank Enum và Bank.acb.
...
class _ServicePaymentScreenState extends State<ServicePaymentScreen> {
  ...
  final Bank _bankEnum = Bank.acb; // Dùng Enum theo thư viện mới
  final String _accountNo = '23071517'; // SỐ TÀI KHOẢN CỦA BẠN
  final String _accountName = 'Nguyễn Minh Thiện';
  final String _bankName = 'Ngân hàng ACB';
  
  // Dữ liệu ĐỘNG
  late final String transferAmount; // Số tiền (dạng String)
  late final String transferContent; // Nội dung chuyển khoản
  late final String vietQrPayload; // Chuỗi dữ liệu VietQR chuẩn
  ...
  void initState() {
    ...
    // Phải dùng .toString() để chuyển giá trị double thành string, nếu healthPackage.price là double
    ...
    try {
      // SỬ DỤNG CÚ PHÁP: VietQR.generate(bank: Bank.acb, accountNumber: ...)
      vietQrPayload = VietQR.generate(
        bank: _bankEnum, // Sử dụng Enum Ngân hàng
        accountNumber: _accountNo, // Số tài khoản
        amount: double.tryParse(transferAmount) ?? 0.0, // Số tiền
        message: transferContent, // Nội dung chuyển khoản
      );
    } catch (e) {
  ...
  void _completeBooking(BuildContext context, String method) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'ĐẶT LỊCH THÀNH CÔNG! Phương thức: $method. Vui lòng kiểm tra email/SMS xác nhận.',
